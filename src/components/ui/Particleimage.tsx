...
export default function ParticleImage(props) {
    ...
    const mouseRef = useRef<any>({ x: -99999, y: -99999, active: false })
    const prevMouseRef = useRef<any>({ x: -99999, y: -99999 })
    const mouseSpeedRef = useRef<number>(0)
    const smoothMouseRef = useRef<any>({ x: -99999, y: -99999 })
    ...
    const dimsRef = useRef<any>({ W: 0, H: 0 })
    ...
    const roamFadeStartRef = useRef<number>(0)
    const roamFadeFromRef = useRef<number>(1)
    const roamFadeToRef = useRef<number>(1)
    ...
    const initParticles = () => {
        ...
        // ❌ REMOVIDO daqui: canvas.width/height, mouseRef reset, sceneRef reset
        // Isso só acontece depois que o novo conteúdo estiver pronto (ver abaixo)
        ...
        const tryLoad = (cors) => {
            ...
            img.onload = () => {
                ...
                if (md === "fit") {
                    ...
                    const w = iw * scaleCover
                    const h = ih * scaleCover
                    rect = { x: (W - w) / 2, y: (H - h) / 2, w, h }
                } else if (sU === "px") {
                ...
                try {
                    ...
                    console.warn('ParticleImage: falha ao ler pixels da imagem (CORS?)', err)
                    return // ✅ não mexe no canvas nem na cena — mantém o que já estava visível
                }
                ...
                if (!src.length) {
                    console.warn('ParticleImage: nenhuma partícula extraída da imagem, mantendo estado anterior')
                    return // ✅ segurança extra: nunca troca por uma cena vazia
                }
                ...
                // ✅ só agora — com tudo pronto e validado — tocamos no canvas e na cena
                canvas.width = Math.round(W * dpr)
                canvas.height = Math.round(H * dpr)
                mouseRef.current = { x: -99999, y: -99999, active: false }
                sceneRef.current = { particles }

                if (reqId === requestIdRef.current && autoActivateRef.current) {
                    setTimeout(() => startAnimRef.current?.("assembling"), 50)
                }
            }
            img.src = url
        }
    ...
    useEffect(() => {
        ...
        const ro = new ResizeObserver((entries) => {
            ...
            // Ignora mudanças pequenas (ex: barra de endereço do mobile recolhendo)
            if (
                prev.W &&
                prev.H &&
                Math.abs(prev.W - W) < 4 &&
    ...
    const onMouseMove = (e) => {
        if (!physicsRef.current.hover) return
        ...
        if (physicsRef.current.hover && !autoActivate) {
    ...
    const onMouseLeave = () => {
        ...
        // No mobile, a imagem deve permanecer montada
        if (autoActivate) return
